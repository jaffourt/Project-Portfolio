function [q,JJ] = sarsa(R, gamma, goalState)

%   - gamma is learning rate 
%     (0 means don't learn anything 1 means 
%     remember all of past)
%   
...
for episode=0:T
    ...
    while state~=goalState            % loop until find goal state
        ...
        if size(x,1)>0
            buffer = RandomPermutation(x);
            rand_state = q(x1, buffer(1));                         
        end
        
        q(state,x1)= R(state,x1) + gamma*rand_state;    % get max of all actions from the next state for Q of current state
        state=x1;
    end
